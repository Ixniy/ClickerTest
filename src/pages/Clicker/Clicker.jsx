...
const Clicker = () => {
  ...
  useEffect(() => {
    if (users && !isInitialized) {
      setEnergy(users.data.energy);
      setStars(users.data.stars);
      console.log(users.status);
      setIsInitialized(true);
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isInitialized, user?.id, users]);


  const handlePressStart = () => {  
    if (energy === 0) return;
    if (Number((stars + 0.2).toFixed(8)) === users.data.stars + 1) {
      ...
      <div className={classes.content}>
        ...
        <div className={classes.staminaProgressContainer}>
          <div className={classes.rankContainer}>
            <span className={classes.rank}>{users && users.data.rank}</span>
            {0 ? (
              <span>Loading...</span>
            ) : (
              <span className={classes.lvl}>lvl {users.data.level}</span>
            )}
          </div>
